...
def _msg2chunks(message, bits):
    '''
    Converts message input to SHA2 chunks.
...
def sha2hash(message, bits):
    ...
    Takes data and digest bits as input, dies if (bits) not in SHA2
    ...
    if bits in (224, 256):
        ...
        rounds = 80
        mod = 64
    chunks = _msg2chunks(message, bits)
